#import "HTJastor.h"
#import "HTJastorRuntimeHelper.h"

@implementation HTJastor
...
- (id)initWithDictionary:(NSDictionary *)dictionary {
	...
	if ((self = [super init])) {
		for (NSString *key in [HTJastorRuntimeHelper propertyNames:[self class]]) {
            ...
            if ([HTJastorRuntimeHelper isPropertyReadOnly:[self class] propertyName:key]) {
                continue;
            }
			
			// handle dictionary
			if ([value isKindOfClass:nsDictionaryClass]) {
				Class klass = [HTJastorRuntimeHelper propertyClassForPropertyName:key ofClass:[self class]];
...
#pragma clang diagnostic pop
				...
				for (id child in value) {
					if ([[child class] isSubclassOfClass:nsDictionaryClass]) {
						HTJastor *childDTO = [[arrayItemType alloc] initWithDictionary:child];
...
- (NSMutableDictionary *)toDictionary {
	...
	for (NSString *key in [HTJastorRuntimeHelper propertyNames:[self class]]) {
        ...
        if (value && [value isKindOfClass:[HTJastor class]]) {            
            ...
            if (internalValue && [internalValue isKindOfClass:[HTJastor class]]) {
...
- (BOOL)isEqual:(id)object {
	if (object == nil || ![object isKindOfClass:[HTJastor class]]) return NO;
	
	HTJastor *model = (HTJastor *)object;
